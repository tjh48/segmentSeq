.processPosteriors <- function(lociPD, nullPD, emptyPD, aD, lociCutoff = 0.9, nullCutoff = 0.9, getLikes = TRUE, cl)
  {
    ...
    if(nrow(nullPD) > 0) selNull <- nullPD[rowSums(sapply(1:ncol(lociPD@locLikelihoods), function(jj) nullPD@locLikelihoods[,jj] >= log(nullCutoff)), na.rm = TRUE) > 0,] else selNull <- nullPD
