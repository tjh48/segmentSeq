...
if(any(abs(colSums(exp(hS@locLikelihoods)) - c(88, 227)) > 2)) stop("hS object contains wrong number of loci. Likely failure.")
@ 
...
if(any(abs(colSums(exp(cS@locLikelihoods)) - c(29,31)) > 2)) stop("cS object contains wrong number of loci. Likely failure.")
@ 
