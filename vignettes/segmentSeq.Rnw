...
<<echo = FALSE, results = hide>>=
if(nrow(sD) != 14444) stop("sD object is the wrong size (should have 14444 rows). Failure.")
@ 
...
<<echo = FALSE, results = hide>>=
if(nrow(hS) != 790) stop("hS object is the wrong size (should have 790 rows). Failure.")
if(any(abs(colSums(exp(hS@locLikelihoods)) - c(179, 394)) > 2)) stop("hS object contains wrong number of loci. Likely failure.")
@ 
...
<<echo = FALSE, results = hide>>=
if(abs(nrow(cS) - 182) > 2) stop("cS object is the wrong size (should have ~182 rows). Likely failure.")
if(any(abs(colSums(exp(cS@locLikelihoods)) - c(76,95)) > 2)) stop("cS object contains wrong number of loci. Likely failure.")
@ 
